...
class Yoko750File(object):
    ...
    def __init__(self, fileName):
        if fileName[-3:]=='HDR' or fileName[-3:]=='WVF' or fileName[-3:]=='hdr' or fileName[-3:]=='wvf':
            fileName=fileName[:-4]
    ...
    def get_header(self):
        ...
        try:
            ...
            numGroups = int(groupLine[1])

            # Get the number of traces
            traceLine = f.readline().split()
            numTraces = int(traceLine[1])
            ...
            dataOffset = float(dataOffsetLine[1])

            traceID = []
            VResolution = []
            VOffset = []

            for j in range(numFullGroups):
            ...
            if (remainingTraces > 0):
                ...
                # first trace entry
                blockSizeString = f.readline().split()
                blockSize = int(blockSizeString[1])
                ...
                VOffsetString = f.readline().split()
                for i in range(remainingTraces):
                    VOffset.append(float(VOffsetString[i+1]))
                ...
                HResolutionString = f.readline().split()
                HResolution = float(HResolutionString[1])
    ...
    def get_trace_data(self, header, traceNumber, timeStart, timeStop, \
        ...
        with open(self.fileName + '.WVF', 'rb') as f:
            ...
            f.seek(int(2 * (blockSize * (traceNumber - 1) + startIndex) + dataOffset), 0)
...
class YokoHeader(object):
    ...
    def get_time_bounds(self):
        return self.HOffset, self.HOffset+self.HResolution*self.blockSize
