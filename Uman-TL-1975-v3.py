...
for i in range(0,Ntz): #for each time t, integrate over z
        ...
        E_ind[i]=(1/(2*eps0*math.pi))*it.simps(E1,dx=dt)
#
#        E2=((np.sin(theta))**2)*dIz[:,i]/(c*c*R)       
#        E_rad[i]=-(1/(2*eps0*math.pi))*it.simps(E2,dx=dt)
#        
#        E3=Iz[:,i]
#        E_elec_total[:,i]=(1/(2*eps0*math.pi))*it.simps(E3,dx=dt)
#        
#        for j in range(0,Nz):
#            print('stuck',i,j)
#            E4=(2-3*((np.sin(theta[j]))**2))/(R[j]*R[j]*R[j])
#            E_elec_total[j,i]=E4#(1/(2*eps0*math.pi))*it.simps(E4,dx=dz)   
#        
print('Ntz',Ntz)
print('tz',tz.shape)
